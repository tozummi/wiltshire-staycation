(() => {
  ...
  function setupViewSwitcher(onChange = null) {
    document.querySelectorAll("[data-view-mode]").forEach(button => {
      button.addEventListener("click", () => {
        viewMode = button.dataset.viewMode === "compact"
          ? "compact"
          : "detailed";
  ...
  function setCompletionMessage(progress, titleElement, copyElement) {
    if (progress.total > 0 && progress.packed === progress.total) {
      titleElement.textContent = "✅ Everything packed!";
      copyElement.textContent = "Have a wonderful holiday 🌿";
      copyElement.classList.add("packing-complete-message");
    } else {
      titleElement.textContent = "Getting ready";
      copyElement.textContent = `${progress.packed} of ${progress.total} packed`;
      copyElement.classList.remove("packing-complete-message");
    }
  }
  ...
  function createCategoryMarkup(category, options = {}) {
    ...
    const itemsMarkup = category.items.map(item => {
      ...
      if (options.household) {
        const assignmentText =
          item.assignedTo || "Who’s bringing it?";

        trailing = `
          <div class="household-row-tools">
            <button
            ...
            ${
              householdEditMode
                ? `
                  <div class="household-item-actions">
                    <button
                      class="household-item-action"
                      type="button"
                      data-edit-household-item="${item.id}"
                      aria-label="Edit ${escapeHTML(item.name)}"
                    >✎</button>

                    <button
                      class="household-item-action delete"
                      type="button"
                      data-delete-household-item="${item.id}"
    ...
    const categoryTools =
      options.household && householdEditMode
        ? `
          <span class="category-edit-actions">
            <span
              ...
              role="button"
              tabindex="0"
              data-rename-household-category="${escapeHTML(category.name)}"
        ...
        >
          ...
          <span class="packing-category-heading">
            <strong>
              ...
              ${isComplete ? `<span class="complete-label">✓</span>` : ""}
            </strong>
            <small>
              ${isComplete ? "Complete" : `${packedCount} of ${total} packed`}
            </small>
          </span>
  ...
  function initialisePersonalTemplateList(type) {
    ...
    function render() {
      ...
      setCompletionMessage(
        ...
        document.getElementById("overall-progress-copy")
      );
  ...
  async function initialiseHouseholdList() {
    ...
    const householdEditToolbar =
      document.getElementById("household-edit-toolbar");

    const toggleHouseholdEdit =
      document.getElementById("toggle-household-edit");

    const addHouseholdItemButton =
      document.getElementById("add-household-item");

    const householdItemModal =
      document.getElementById("household-item-modal");

    const householdItemForm =
      document.getElementById("household-item-form");

    const householdItemModalTitle =
      document.getElementById("household-item-modal-title");

    const householdItemId =
      document.getElementById("household-item-id");

    const householdItemName =
      document.getElementById("household-item-name");

    const householdItemCategory =
      document.getElementById("household-item-category");

    const householdItemAssigned =
      document.getElementById("household-item-assigned");

    const householdCategoryOptions =
      document.getElementById("household-category-options");

    householdEditToolbar.hidden = false;
    ...
    function transformRows(rows) {
      ...
      return categoryNames.map(categoryName => {
        const templateCategory =
          HOUSEHOLD_TEMPLATE.categories.find(
    ...
    function render() {
      ...
      document.body.classList.toggle("household-editing", householdEditMode);
    ...
    async function loadItems() {
      ...
      const categoryNames = [
        ...new Set(
          householdItems.map(item => item.category)
        )
      ];

      householdCategoryOptions.innerHTML =
        categoryNames
    ...
    categoryContainer.addEventListener("click", async event => {
      ...
      const editButton = event.target.closest("[data-edit-household-item]");

      if (editButton && householdEditMode) {
        const item = householdItems.find(entry => entry.id === editButton.dataset.editHouseholdItem);
        if (!item) return;

        householdItemForm.reset();
        householdItemId.value = item.id;
        householdItemName.value = item.item;
        householdItemCategory.value = item.category;
        householdItemAssigned.value = item.assigned_to || "";
        householdItemModalTitle.textContent = "Edit Household Item";
        openModal(householdItemModal);
        return;
      }

      const deleteButton = event.target.closest("[data-delete-household-item]");

      if (deleteButton && householdEditMode) {
        const item = householdItems.find(entry => entry.id === deleteButton.dataset.deleteHouseholdItem);
        if (!item) return;

        showConfirm({
          title: "Delete this household item?",
          message: `“${item.item}” will be removed from the shared list for everyone.`,
          actionLabel: "Delete",
          onConfirm: async () => {
            const { error } = await householdClient
              .from("household_packing_items")
            ...
            if (error) {
              console.error(error);
              showToast("The household item could not be deleted.", true);
              return;
            }

            await loadItems();
            showToast("Household item deleted.");
          }
        });
      ...
      const renameCategoryButton =
        event.target.closest("[data-rename-household-category]");

      if (renameCategoryButton && householdEditMode) {
        const oldName = renameCategoryButton.dataset.renameHouseholdCategory;
        const newName = window.prompt("Rename this category:", oldName)?.trim();
        ...
        if (error) {
          console.error(error);
          showToast("The category could not be renamed.", true);
          return;
        }

        await loadItems();
        showToast("Household category renamed.");
      }
    });
    ...
    toggleHouseholdEdit.addEventListener("click", () => {
      ...
      toggleHouseholdEdit.textContent =
        householdEditMode
          ? "✓ Done Editing"
          : "✎ Edit Household List";

      addHouseholdItemButton.hidden = !householdEditMode;
      render();
    });

    addHouseholdItemButton.addEventListener("click", () => {
      householdItemForm.reset();
      householdItemId.value = "";
      householdItemModalTitle.textContent = "Add Household Item";

      const firstCategory =
        householdItems[0]?.category ||
        HOUSEHOLD_TEMPLATE.categories[0]?.name ||
        "Household Essentials";

      householdItemCategory.value = firstCategory;
      openModal(householdItemModal);
    });

    householdItemForm.addEventListener("submit", async event => {
      event.preventDefault();

      const id = householdItemId.value;
      const category = householdItemCategory.value.trim();
      const itemName = householdItemName.value.trim();
      const assignedTo = householdItemAssigned.value.trim() || null;

      if (!category || !itemName) return;

      const existingCategoryRows =
        householdItems.filter(item => item.category === category);

      const knownTemplateIndex =
        HOUSEHOLD_TEMPLATE.categories.findIndex(entry => entry.name === category);

      const categoryOrder =
        existingCategoryRows[0]?.category_order ||
        (knownTemplateIndex >= 0 ? knownTemplateIndex + 1 : 99);
      ...
      if (id) {
        ...
        result = await householdClient
          ...
          .insert({
            ...payload,
            display_order: existingCategoryRows.length + 1,
            packed: false
          });
      }
      ...
      if (result.error) {
        ...
        showToast("The household item could not be saved.", true);
        return;
      }

      closeModal(householdItemModal);
      await loadItems();
      showToast(id ? "Household item updated." : "Household item added.");
    });
  ...
  function initialiseCustomPage() {
    ...
    function renderEditor() {
      ...
      const customProgressTitle =
        document.getElementById("custom-progress-title");

      const customProgressCopy =
        document.getElementById("custom-progress-copy");

      if (progress.total > 0 && progress.packed === progress.total) {
        customProgressTitle.textContent = "✅ Everything packed!";
        customProgressCopy.textContent = "Have a wonderful holiday 🌿";
        customProgressCopy.classList.add("packing-complete-message");
      } else {
        customProgressTitle.textContent = currentCustomList.name;
        customProgressCopy.textContent = `${progress.packed} of ${progress.total} packed`;
        customProgressCopy.classList.remove("packing-complete-message");
      }
